...
```{r}
long_sims <- read_rds("sim_output/long_sims_20240612_103629.rds")
...
ggplot(long_summary, aes(x = time, y = disp_cur, color = prop)) +
  geom_point() +
...
ggplot(cors, aes(x = time, y = p.value, color = prop)) +
  geom_point() +
  facet_grid(rows = vars(cluster), cols = vars(recov_prob)) +
  theme_bw()
```
