---
title: "sdp_function_prep_notebook"
output: html
---
...
point <- sample(coords$id, size = 1)

coords[coords$id %in% point, "value"] <- 1

point_index <- which(coords$id == point)

dists <- 1/as.matrix(dist.torus(cbind(coords$x, coords$y)))[point_index,]

coords$weight <- dists
```

Wait! What if I use the dist function instead of loops?
...
inv_dists_filt <- inv_dists[, ones]

coords$weight <- map_dbl(1:nrow(inv_dists_filt), function(x){max(inv_dists_filt[x,])})
```
...
Sys.time()
test_ls <- fast_ls_perc(size = 50, prop = 0.25, cluster = 10)
Sys.time()

plot(raster(test_ls$matrix))

walk(seq(0,100,10), function(x){
  plot(raster(fast_ls_perc(size = 25, prop = 0.25, cluster = x)$matrix))})

```

It works! Much faster. But can it be made faster by changing the for loop to a while loop and modifying the rate at which cells are filled from one at a time to higher? Maybe not a while loop...
...
fast_ls_perc <- function(size, prop, cluster, rate = 1, batch_cluster){
  ## Calculate number of cells to fill based on size and prop
  potential <- floor((size^2) * prop)
  ## Calculate number of iterations
  iters <- ceiling(potential / rate)
  ## Save remainder
  remainder <- potential %% rate
  ## Create empty data representing the landscape
  coords <- tibble(x = rep(1:size, size), 
                   y = rep(1:size, each = size),
                   id = str_c(x, y, sep = "_"),
                   weight = rep(1, size^2), # Weight starts equal for all cells
                   value = rep(0, size^2), # Cell type starts out as 0 for all
                   batch_start = rep(0, size^2),
                   batch_weight = rep(1, size^2)) 
  
  ## Initialize the possible percolation locations and ones
  possibles <- coords$id
  ones <- coords$value == 1
  ## Percolate as many cells as need to be filled
  for(p in seq_len(iters)){
    ...
    if(rate > 1){
      coords[coords$id %in% point, "batch_start"] <- 1
      batch_size <- if_else(potential - rate > sum(ones), rate, remainder)
      batch_size <- if_else(batch_size > 1, batch_size - 1, batch_size)
      ...
      ## Select only the column of the batch start point
      inv_dists_filt <- as.matrix(inv_dists[,coords$batch_start == 1])
      
      ## Save weights
      coords$batch_weight <- map_dbl(1:nrow(inv_dists_filt), 
                               function(x){
                                (1 + max(inv_dists_filt[x,]))^batch_cluster
                               })
      
      batch_points <- sample(possibles, size = batch_size, prob = coords[coords$id %in% possibles,]$batch_weight)
      coords[coords$id %in% batch_points, "value"] <- 1
      
      coords$batch_start <- 0
    }
...
Sys.time()
test_ls <- fast_ls_perc(size = 48, prop = 0.25, cluster = 10, rate = 1)
Sys.time()
...
walk(c(1, seq(10,100,10)), function(x){
  plot(raster(fast_ls_perc(size = 30, prop = 0.25, cluster = 10, rate = x, batch_cluster = 25)$matrix))})

```

Trying to figure out a batch clustering to clustering ratio...

for size 30, prop 0.25, cluster 10

batch cluster 15 for rate 10

batch cluster 25 for rate 30

... I don't think a perfect ratio can be found, it may be best to let cluster and batch cluster freely vary.

Also, my function can't do large landscapes (e.g. 256 x 256)

I'm going to leave optimizing this further for now, but perhaps when revisiting it in the future I could constrain the possible cells for each batch to some number of the closest cells to the seed cell.

#### Revisiting Movement Simulation

First, let's look at the movement functions I made last summer from "clustering_sim_wip.R"
...
Might need a complete rewrite for vectorizing...

```{r}

## set init coords
host <- tibble(x = 9, y = 4, id = 1)

## random moves
xmoves <- sample(c(-1, 0, 1), size = 20, replace = TRUE)
ymoves <- sample(c(-1, 0, 1), size = 20, replace = TRUE)

## calculate x positions
xpos <- cumsum(c(host$x, xmoves))

## wrap around for torus
xpos[xpos < 1] <- xpos[xpos < 1] + 10
xpos[xpos > 10] <- xpos[xpos > 10] - 10

## calculate x positions
ypos <- cumsum(c(host$y, ymoves))

## wrap around for torus
ypos[ypos < 1] <- ypos[ypos < 1] + 10
ypos[ypos > 10] <- ypos[ypos > 10] - 10

xpos

## added later... needs coords

host <- host %>%
  bind_rows(tibble(x = xpos, y = ypos))

host_moved <- host %>%
    bind_rows(tibble(x = xpos,
                     y = ypos,
                     time = 1:21)) %>%
    mutate(id = str_c(x, y, sep = "_")) %>% ## generate ids
    left_join(select(coords, id, value), by = "id") %>% ## bring in parasite info
    mutate(parasite_gain = value * rbinom(n = n(), size = 1, prob = 0.75),
           parasite_burden = cumsum(parasite_gain))

length(cumsum(host_moved$parasite_gain))

host_moved <- tibble(x = xpos,
                       y = ypos,
                       time = 0:20) %>%
    mutate(id = str_c(x, y, sep = "_")) %>% ## generate ids
    left_join(select(coords, id, value), by = "id") ## bring in parasite info
...
Okay, so that code essentially vectorizes movement, but how do I scale it up?

```{r}

## first, let's make a landscape coords tibble with random parasites
size <- 12
coords <- tibble(x = rep(1:size, size), 
                   y = rep(1:size, each = size),
                   id = str_c(x, y, sep = "_"),
                   weight = rep(1, size^2), # Weight starts equal for all cells
                   value = rbinom(n = size^2, size = 1, p = 0.25))


## then let's generate starting positions for hosts
## from move_sim1:
n_hosts <- 24
host_origins <- slice_sample(coords, n = n_hosts, replace = TRUE)
...
## i think next would be a function that takes one host, moves it, then checks for parasites
...
move_host <- function(hid, hosts, coords, n_moves){
  ...
  ## wrap around for torus
  xpos[xpos < 1] <- xpos[xpos < 1] + max_x
  xpos[xpos > max_x] <- xpos[xpos > max_x] - max_x
  
  ## calculate y positions
  ypos <- cumsum(c(host_start$y, ymoves))
  ## wrap around for torus
  ypos[ypos < 1] <- ypos[ypos < 1] + max_y
  ypos[ypos > max_y] <- ypos[ypos > max_y] - max_y
...
test_host <- move_host(hid = 1, hosts = hosts, coords = coords, n_moves = 100)

test_hosts <- map_df(1:24, ~move_host(hid = .x, hosts = hosts, coords = coords, n_moves =  100))

range(test_hosts$y)

```

Works great! Maybe a matrix based implementation as opposed to a functional implementation would be faster, but this is plenty fast

#### Setting up the Simulation

Now set up functions to run through a list of landscapes
...
landscape_list <- map(c(0, 5, 10),
                      ~fast_ls_perc(size = 12, prop = 0.2, 
                                    cluster = .x))

range(landscape_list[[3]]$coords$y)


test_sims <- map_df(landscape_list,
                    ~parasite_sim(.x, n_host = 24, n_moves = 100))

test_sim_summary <- test_sims %>%
  filter(time == max(time)) %>%
  group_by(moran) %>%
  summarize(mean_par = mean(parasite_burden),
            var_par = var(parasite_burden),
            disp_par = var_par / mean_par)
```

Turns out my movement simulator wasn't working! So I made the function above save more info
...
test_host2 <- test_host %>%
  mutate(oob_x = (pre_x - 1) %/% max_x,
         oob_y = (pre_y - 1) %/% max_y,
         true_x = pre_x - (max_x * oob_x),
         true_y = pre_y - (max_y * oob_y))

range(test_host2$true_y)
range(test_host2$true_x)

## [old]this can't deal with positions at the max (12)...
## this now works!

```

#### Setting up the Sim, Redux

let's redo the functions
