...
move_host <- function(hid, hosts, coords, nmoves){
  ## grab host
  ...
  ## random moves
  xmoves <- sample(c(-1, 0, 1), size = nmoves, replace = TRUE)
  ymoves <- sample(c(-1, 0, 1), size = nmoves, replace = TRUE)
...
test_host <- move_host(hid = 1, hosts = hosts, coords = coords, nmoves = 20)

test_hosts <- map_df(1:24, ~move_host(hid = .x, hosts = hosts, coords = coords, nmoves =  1000))
