...
## this figure has "snapshot" burden dispersion on y axis, spatial aggregation on x axis, parasite density as color, but no panels
disp2_fig <- ggplot(sim_summary, aes(x = moran, y = disp_cur, color = factor(prop))) +
  geom_point(size = 3) +
  geom_smooth(se = FALSE, method = "lm", linewidth = 2) +
  scale_color_viridis_d(name = "Parasite Density \n(Proportion Cells Occupied)") +
  labs(y = "Parasite Aggregation on Hosts\n(Variance : Mean Ratio)") +
  scale_x_continuous(name = "Parasite Spatial Clustering (Moran's I)",
                     breaks = c(0, 0.06, 0.12, 0.18)) +
  theme_bw() +
  theme(legend.position = "bottom",
        legend.title = element_text(size = 24),
...
## save simple dispersion fig
ggsave(str_c("figures/sap_simple_disp_", timestamp, ".tiff"), plot = disp2_fig,
       units = "in", width = 12, height = 8)
