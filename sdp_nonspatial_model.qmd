...
nostay_sim_data <- read_rds("sim_output/nostay_sim_data20240611_130259.rds")

stay_summary <- recov_sim_data %>%
  filter(cluster == 0) %>%
...
nostay_summary <- nostay_sim_data%>%
  filter(cluster == 0) %>%
  group_by(prop, rep_id, recov_prob) %>%
  summarize(mean_cur = mean(cur_burden),
            mean_cum = mean(cum_burden),
...
## combine data
comp_summary <- stay_summary %>%
  mutate(sim_type = "spatial_w_stay") %>%
  bind_rows(ns_summary %>% mutate(sim_type = "non_spatial"),
            nostay_summary %>% mutate(sim_type = "spatial_no_stay"))


ggplot(data = comp_summary, aes(x = prop, y = var_cur, color = sim_type)) +
  geom_jitter(width = 0.01) +
  geom_smooth(method = "lm", se = FALSE) +
  facet_wrap(vars(recov_prob), nrow = 3) +
  theme_bw()
...
ggplot(data = recov_sim_data %>% filter(cluster == 0), aes(x = cum_burden)) +
  geom_histogram() +
...
ggplot(data = nostay_sim_data, aes(x = cum_burden)) +
  geom_histogram() +
  facet_grid(rows = vars(prop), cols = vars(recov_prob))
```
